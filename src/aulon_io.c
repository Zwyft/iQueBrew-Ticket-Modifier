...
int get_input(char *line_buffer, int buffer_length, FILE *instream) {
  ...
  // If fgets returned after hitting limit, discard extra characters in
  // instream. Otherwise it returned after receiving a newline; replace it with
  // null.
...
int open_file(FILE **file, const char *filename, const char *mode) {
  if (file == NULL || filename == NULL || mode == NULL) {
    fprintf(stderr, "ERROR: NULL argument(s) provided to open_file(). Opening "
                    "file aborted.\n");
    return 0;
  }
