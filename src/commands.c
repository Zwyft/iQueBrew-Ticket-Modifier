...
int read_block_spare(unsigned char *block_buffer, unsigned char *spare_buffer,
...
static int request_block_read(uint32_t command, uint32_t block_number) {
  if (!ique_send_command(command, block_number)) {
    fprintf(stderr,
            "Command to read block 0x%04x was sent, but it was not received by "
            "the console.\n",
            block_number);
    return 0;
  }
  ...
  if (command_error(reply_buffer)) {
    fprintf(
        stderr,
        "The console returned an error when requesting to read block 0x%04x.\n",
        block_number);
    return 0;
  }
...
int write_block_spare(unsigned char *block_buffer, unsigned char *spare_buffer,
                      uint32_t block_number) {
...
static int request_block_write(uint32_t command, uint32_t block_number) {
  if (!ique_send_command(command, block_number)) {
    fprintf(stderr,
            "Command to write block 0x%04x was sent, but it was not received "
            "by the console.\n",
            block_number);
    return 0;
  }
...
static int check_block_write(uint32_t block_number) {
  unsigned char reply_buffer[8] = {0};
  if (!ique_receive_reply(reply_buffer, 8)) {
    fprintf(stderr, "Error after writing block: Console did not send success "
                    "state response.\n");
    return 0;
  }
  ...
  if (command_error(reply_buffer)) {
    fprintf(stderr,
            "The console returned an error after writing block 0x%04x.\n",
            block_number);
    return 0;
  }
...
int init_fs(void) {
  if (!ique_send_command(INIT_FS, 0x0000)) {
    fprintf(
        stderr,
        "INIT_FS command was sent, but it was not received by the console.\n");
    return 0;
  }
  ...
  if (command_error(reply_buffer)) {
    fprintf(stderr,
            "The console returned an error when initializing the FS.\n");
    return 0;
  }
...
int get_num_blocks(void) {
  if (!ique_send_command(GET_NUM_BLOCKS, 0x0000)) {
    fprintf(stderr, "GET_NUM_BLOCKS command was sent, but it was not received "
                    "by the console.\n");
    return 0;
  }
  ...
  if (!ique_receive_reply(reply_buffer, 8)) {
    fprintf(stderr,
            "Console response to GET_NUM_BLOCKS command not received.\n");
    return 0;
  }

  if (uchars_to_uint32(&reply_buffer[4]) != 0x1000) {
    fprintf(stderr,
            "The current iQue NAND does not contain exactly 0x1000 blocks.\n");
    fprintf(stderr, "aulon does not currently support cards with such sizes, "
                    "and will abort.\n");
    return 0;
  }
...
int set_seqno(uint32_t arg) {
  if (!ique_send_command(SET_SEQNO, arg)) {
    fprintf(stderr, "SET_SEQNO command was sent, but it was not received by "
                    "the console.\n");
    return 0;
  }
...
int get_seqno(void) {
  if (!ique_send_command(GET_SEQNO, 0x0000)) {
    fprintf(stderr, "GET_SEQNO command was sent, but it was not received by "
                    "the console.\n");
    return 0;
  }
  ...
  if (command_error(reply_buffer)) {
    fprintf(
        stderr,
        "The console returned an error in response to GET_SEQNO command.\n");
    return 0;
  }
...
/*
    file_checksum_cmp
    Sends a filename, checksum, and file size to the console. The console
   returns zero if the file on the console with the given filename has a
   matching checksum and size, or less-than-zero otherwise.
*/
int file_checksum_cmp(const char *filename, uint32_t checksum, uint32_t size) {
  return send_filename(filename) &&
         send_params_and_receive_reply(checksum, size);
}

static int send_filename(const char *filename) {
  uint32_t fn_len = strlen(filename) + 1;
  if (fn_len > 13) {
    fprintf(stderr, "The given filename is invalid; it is too long for the "
                    "iQue Player FS.\n");
    return 0;
  }

  if (!ique_send_command(FILE_CHKSUM, fn_len)) {
    fprintf(stderr, "FILE_CHKSUM command was sent, but it was not received by "
                    "the console.\n");
    return 0;
  }
  ique_wait_for_ready();
...
static int send_params_and_receive_reply(uint32_t checksum, uint32_t size) {
  ...
  if (!ique_send_command(checksum, size)) {
    fprintf(stderr,
            "Error sending potential checksum and file size to the console.\n");
    return 0;
  }
...
int set_led(uint32_t arg) {
  if (!ique_send_command(SET_LED, arg)) {
    fprintf(
        stderr,
        "SET_LED command was sent, but it was not received by the console.\n");
    return 0;
  }
...
int set_time(uint32_t first_half, unsigned char *second_half) {
  if (!ique_send_command(SET_TIME, first_half)) {
    fprintf(
        stderr,
        "SET_TIME command was sent, but it was not received by the console.\n");
    return 0;
  }
...
int get_bbid(uint32_t *bbid_out) {
  if (!ique_send_command(GET_BBID, 0x0000)) {
    fprintf(stderr,
            "BBID request was sent, but it was not received by the console.\n");
    return 0;
  }
  ...
  if (command_error(reply_buffer)) {
    fprintf(
        stderr,
        "The console returned an error when requesting the console's ID.\n");
    return 0;
  }
...
int sign_hash(unsigned char *hash_in, unsigned char *sig_out) {
  if (!ique_send_command(SIGN_HASH, SHA1_HASH_LENGTH)) {
    fprintf(stderr, "Request to sign hash was sent, but it was not received by "
                    "the console.\n");
    return 0;
  }
  ...
  if (command_error(reply_buffer)) {
    fprintf(
        stderr,
        "The console returned an error in response to sign_hash request.\n");
    return 0;
  }
