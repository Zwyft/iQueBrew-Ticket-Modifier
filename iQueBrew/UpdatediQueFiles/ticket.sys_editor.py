...
class OwO_UwU(Tk): # Just a random name choice, no reason behind it at all
	...
	def __init__(self):
		...
		self.appInfoCentre = Frame(self.appInfo)
		self.appInfoCentre.pack()
		
		self.ticketCentre = Frame(self.ticket)
		self.ticketCentre.pack()
	...
	def setupWidgets(self):
		...
		for i in range(len(self.ticketSplitNames["en"])):
			...
			if i < 18:
				...
				self.widgets.append(LabelledEntry(self.ticketCentre, textvariable = self.splitnamevars[i], length = sectionLength))				
		...
		self.widgets[20:22] = [ButtonImage(self.appInfoCentre, textvariable = self.textvars[36], command = self.selectImage, bg = "#333366"), ButtonImage(self.appInfoCentre, textvariable = self.textvars[37], command = lambda: self.selectImage(isTitle = True), bg = "#333366")]
		...
		self.widgets[36] = LabelledEntry(self.appInfoCentre, textvariable = self.textvars[39])

		self.widgets[40] = ButtonEntry(self.ticketCentre, textvariable = self.textvars[40], command = self.editTicketID, state = "readonly")
